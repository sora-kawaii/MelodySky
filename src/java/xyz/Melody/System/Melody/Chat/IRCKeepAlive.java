...
public final class IRCKeepAlive {
    ...
    public void chouShabi(EventTick eventTick) {
        ...
        if (this.timer.hasReached(60000.0) && !Client.instance.ircExeption && IRC.pw != null) {
            ...
            IRC.pw.println("INGAME_VERIFY: " + Minecraft.getMinecraft().getSession().getUsername() + "@" + Client.instance.authManager.verified + "@" + Client.mc.getSession().getProfile().getId().toString() + "@" + this.lineString() + "@" + "MelodySky" + "@" + "2.5.1");
            this.timer.reset();
        }
        this.ticks = 0;
    }
