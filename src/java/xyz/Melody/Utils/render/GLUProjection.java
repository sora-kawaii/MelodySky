...
public final class GLUProjection {
   ...
   public void updateMatrices(IntBuffer viewport, FloatBuffer modelview, FloatBuffer projection, double widthScale, double heightScale) {
      ...
      this.fovX = (float)Math.toDegrees(2.0 * Math.atan(this.displayWidth / this.displayHeight * Math.tan(Math.toRadians((double)this.fovY) / 2.0)));
      new Vector3D((double)this.modelview.get(12), (double)this.modelview.get(13), (double)this.modelview.get(14));
      Vector3D lv = new Vector3D((double)this.modelview.get(0), (double)this.modelview.get(1), (double)this.modelview.get(2));
      Vector3D uv = new Vector3D((double)this.modelview.get(4), (double)this.modelview.get(5), (double)this.modelview.get(6));
      Vector3D fv = new Vector3D((double)this.modelview.get(8), (double)this.modelview.get(9), (double)this.modelview.get(10));
      ...
      this.frustumPos = new Vector3D((double)modelviewMatrix.m30, (double)modelviewMatrix.m31, (double)modelviewMatrix.m32);
      this.frustum = this.getFrustum(this.frustumPos.x, this.frustumPos.y, this.frustumPos.z, yaw, pitch, (double)fov, 1.0, this.displayWidth / this.displayHeight);
      this.invFrustum = this.getFrustum(this.frustumPos.x, this.frustumPos.y, this.frustumPos.z, yaw - 180.0, -pitch, (double)fov, 1.0, this.displayWidth / this.displayHeight);
         ...
         if (outsideFrustum) {
            ...
            if (!extrudeInverted || outsideInvertedFrustum && (!outsideInvertedFrustum || clampModeOutside == GLUProjection.ClampMode.NONE)) {
               if (!GLU.gluProject((float)x, (float)y, (float)z, this.modelview, this.projection, this.viewport, this.coords)) {
                  return new Projection(0.0, 0.0, GLUProjection.Projection.Type.FAIL);
               } else {
            ...
            } else if (extrudeInverted && !outsideInvertedFrustum || clampModeOutside == GLUProjection.ClampMode.DIRECT && outsideInvertedFrustum) {
               guiX = 0.0;
               guiY = 0.0;
               if (!GLU.gluProject((float)x, (float)y, (float)z, this.modelview, this.projection, this.viewport, this.coords)) {
                  return new Projection(0.0, 0.0, GLUProjection.Projection.Type.FAIL);
                  ...
                  if (opposite) {
                     guiX = this.displayWidth * this.widthScale - (double)this.coords.get(0) * this.widthScale - this.displayWidth * this.widthScale / 2.0;
                     guiY = this.displayHeight * this.heightScale - (this.displayHeight - (double)this.coords.get(1)) * this.heightScale - this.displayHeight * this.heightScale / 2.0;
                  } else {
                     guiX = (double)this.coords.get(0) * this.widthScale - this.displayWidth * this.widthScale / 2.0;
                     guiY = (this.displayHeight - (double)this.coords.get(1)) * this.heightScale - this.displayHeight * this.heightScale / 2.0;
                  }

                  Vector3D vec = (new Vector3D(guiX, guiY, 0.0)).snormalize();
                  guiX = vec.x;
                  guiY = vec.y;
                  Line vectorLine = new Line(this.displayWidth * this.widthScale / 2.0, this.displayHeight * this.heightScale / 2.0, 0.0, guiX, guiY, 0.0);
                  ...
                  Vector3D intersect = angle >= this.bra && angle < this.tra ? this.rb.intersect(vectorLine) : (angle >= this.tra && angle < this.tla ? this.tb.intersect(vectorLine) : (angle >= this.tla && angle < this.bla ? this.lb.intersect(vectorLine) : this.bb.intersect(vectorLine)));
