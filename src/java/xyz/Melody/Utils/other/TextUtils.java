...
public class TextUtils {
   ...
   public static String convertMagnitudes(String text) throws ParseException {
      ...
      StringBuffer sb;
      double parsedDouble;
      for(sb = new StringBuffer(); matcher.find(); matcher.appendReplacement(sb, NUMBER_FORMAT.format(parsedDouble))) {
         parsedDouble = NUMBER_FORMAT.parse(matcher.group(1)).doubleValue();
   ...
   public static String reverseText(String originalText) {
      ...
      for(int i = parts.length; i > 0; --i) {
         ...
         char[] var6 = textPart.toCharArray();
         int var7 = var6.length;

         for(int var8 = 0; var8 < var7; ++var8) {
            char letter = var6[var8];
            if (letter > 191) {
               foundCharacter = true;
   ...
   public static String abbreviate(int number) {
      if (number < 0) {
         ...
         Map.Entry entry = suffixes.floorEntry(number);
         Integer divideBy = (Integer)entry.getKey();
