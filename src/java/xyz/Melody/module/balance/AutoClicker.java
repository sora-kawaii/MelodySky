...
extends Module {
    ...
    private void onLMB(EventTick e) {
        ...
        if (Mouse.isButtonDown(0) && this.timer.delay(ldelay) && this.mc.currentScreen == null && ((Boolean)this.left.getValue()).booleanValue()) {
    ...
    private void onRMB(EventTick event) {
        ...
        if (Mouse.isButtonDown(1) && this.timer.delay(rdelay) && this.mc.currentScreen == null && ((Boolean)this.right.getValue()).booleanValue()) {
            Client.rightClick();
            this.timer.reset();
        }
    }
