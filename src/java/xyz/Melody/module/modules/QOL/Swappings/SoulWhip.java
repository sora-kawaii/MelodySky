...
extends Module {
    ...
    private void onMouse(EventTick e) {
        ...
        int n = dik = this.mode.getValue() == ddd.Left ? 0 : 1;
        if (Mouse.isButtonDown(dik) && (((Boolean)this.mouse.getValue()).booleanValue() || ((Boolean)this.both.getValue()).booleanValue())) {
    ...
    private void onKey(EventKey event) {
        if (event.getKey() == this.getKey() && (!((Boolean)this.mouse.getValue()).booleanValue() || ((Boolean)this.both.getValue()).booleanValue())) {
            this.shouldSwitch = true;
