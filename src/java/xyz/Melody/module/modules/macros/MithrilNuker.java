...
public class MithrilNuker extends Module {
   ...
   public MithrilNuker() {
      ...
      this.addValues(new Value[]{this.sneak, this.protect, this.rot});
   ...
   private void destoryBlock(EventPreUpdate event) {
      if (this.mc.thePlayer != null && this.mc.theWorld != null) {
         if (this.playerWithin20B() && (Boolean)this.protect.getValue()) {
            ...
            Helper.sendMessage("[Mithril Fucker] Player Detected in 30 Blocks, Auto Disabled.");
   ...
   private boolean playerWithin20B() {
      ...
      } while(((AntiBot)Client.instance.getModuleManager().getModuleByClass(AntiBot.class)).isEntityBot(player) || !(this.mc.thePlayer.getDistanceToEntity(player) < 20.0F) || player == this.mc.thePlayer);
   ...
   private boolean isMithril(IBlockState blockState) {
      if (blockState.getBlock() == Blocks.prismarine) {
         ...
         return blockState.getBlock() == Blocks.stone && blockState.getValue(BlockStone.VARIANT) == EnumType.DIORITE_SMOOTH;
      }
   }
