...
extends Module {
    ...
    private BlockPos closestStone() {
        ...
        Vec3i vec3i = new Vec3i(r, (Double)this.height.getValue(), r);
        Vec3i depthVec = new Vec3i(r, 1.0, r);
        ...
        if (playerPos != null) {
            for (BlockPos blockPos : BlockPos.getAllInBox(playerPos.add(vec3i), playerPos.subtract(depthVec))) {
                ...
                if (!((Boolean)this.ores.getValue()).booleanValue() || blockState.getBlock() != Blocks.coal_ore && blockState.getBlock() != Blocks.diamond_ore && blockState.getBlock() != Blocks.gold_ore && blockState.getBlock() != Blocks.redstone_ore && blockState.getBlock() != Blocks.iron_ore && blockState.getBlock() != Blocks.lapis_ore && blockState.getBlock() != Blocks.emerald_ore && blockState.getBlock() != Blocks.netherrack || this.broken.contains(blockPos)) continue;
                stones.add(new Vec3((double)blockPos.getX() + 0.5, blockPos.getY(), (double)blockPos.getZ() + 0.5));
            }
        }
