...
extends Module {
    ...
    private void onTick(EventTick event) {
        ...
        if ((this.mc.thePlayer.isBlocking() || this.mc.thePlayer.isUsingItem()) && !(this.mc.theWorld.getBlockState(this.mc.objectMouseOver.getBlockPos()).getBlock() instanceof BlockAir)) {
            int idk2;
            int n = this.mc.thePlayer.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.mc.thePlayer.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (idk2 = this.mc.thePlayer.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.mc.thePlayer.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6);
            if (this.mc.gameSettings.keyBindAttack.isKeyDown() && (!this.mc.thePlayer.isSwingInProgress || this.mc.thePlayer.swingProgressInt >= idk2 / 2 || this.mc.thePlayer.swingProgressInt < 0)) {
